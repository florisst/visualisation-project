...
function updateTooltip(i,x,day_results){
  heat = []
  d = day_results[i]
  ...
  for (var x = 0; x < 6; x+=1) {
    ...
    if (ploeg_heat[2] === veld)
      crewExists(ploeg_heat, heat)
    if (ploeg2_heat[2] === veld)
  ...
  for (test = 0; test < heat.length; test++){
    ...
    if (position == 1)
      var tijd = ploeg_overview[5]["results"]["finish"]["time"]
    console.log(tijd)
  ...
  div.html("<br>" + "Winnende tijd: " + tijd  + "<br>")
